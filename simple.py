...
class SIMPLE(object):
  ...
  def momento_x(self, u_old, v_old, w_old, velocity_iter):
    ...
    area_north = (tf.gather(self.x, ns_indices_x + 1) - tf.gather(self.x, ns_indices_x + 1)) / 2 * \
                 (tf.gather(self.z, ns_indices_z) + tf.gather(self.z, ns_indices_z + 1)) / 2;
    area_south = (tf.gather(self.x, ns_indices_x + 1) - tf.gather(self.x, ns_indices_x + 1)) / 2 * \
                 (tf.gather(self.z, ns_indices_z) + tf.gather(self.z, ns_indices_z + 1)) / 2;
    ...
    An_tail = tf.math.maximum(-flow_north, 0) + self.mu * area_north / ((tf.gather(self.y, ns_tail_indices_y) - tf.gather(self.y, ns_tail_indices_y - 1)) * tf.gather(self.x, ns_indices_x) / 2); # An_tail.shape = (nx-2, 1, nz-1)
    As_head = tf.math.maximum(flow_south, 0) + self.mu * area_south / ((tf.gather(self.y, ns_head_indices_y) - tf.gather(self.y, ns_head_indices_y - 1)) * tf.gather(self.x, ns_indices_x) / 2); # An_head.shape = (nx-2, 1, nz-1)
    ...
    area_top = (tf.gather(self.y, tb_indices_y + 1) - tf.gather(self.y, tb_indices_y)) * \
               (((tf.gather(self.x, tb_indices_x) + tf.gather(self.x, tb_indices_x + 1))/2)**2 - \
                ((tf.gather(self.x, tb_indices_x) + tf.gather(self.x, tb_indices_x - 1))/2)**2) / 2;
    area_bottom = (tf.gather(self.y, tb_indices_y + 1) - tf.gather(self.y, tb_indices_y)) * \
                  (((tf.gather(self.x, tb_indices_x) + tf.gather(self.x, tb_indices_x + 1))/2)**2 - \
                   ((tf.gather(self.x, tb_indices_x) + tf.gather(self.x, tb_indices_x - 1))/2)**2) / 2;
  ...
  def momento_z(self, u_old, v_old, w_old, velocity_iter):
    ...
    area_north = (tf.gather(self.x, ns_indices_x + 1) - tf.gather(self.x, ns_indices_x)) * (tf.gather(self.z, ns_indices_z + 1) - tf.gather(self.z, ns_indices_z - 1)) / 2;
    area_south = (tf.gather(self.x, ns_indices_x + 1) - tf.gather(self.x, ns_indices_x)) * (tf.gather(self.z, ns_indices_z + 1) - tf.gather(self.z, ns_indices_z - 1)) / 2;
    ...
    An_tail = tf.math.maximum(-flow_north, 0) + self.mu * area_north / ((tf.gather(self.y, ns_tail_indices_y) - tf.gather(self.y, ns_tail_indices_y - 1)) * tf.gather(self.x, ns_indices_x) / 2);
    As_head = tf.math.maximum(flow_south, 0) + self.mu * area_south / ((tf.gather(self.y, ns_head_indices_y) - tf.gather(self.y, ns_head_indices_y - 1)) * tf.gather(self.x, ns_indices_x) / 2);
    ...
    area_east = tf.gather(self.x, ew_tail_indices_x) / 2 * (tf.gather(self.y, ew_indices_y + 1) - tf.gather(self.y, ew_indices_y)) * (tf.gather(self.z, ew_indices_z + 1) - tf.gather(self.z, ew_indices_z - 1)) / 2;
    area_west = tf.gather(self.x, ew_head_indices_x) / 2 * (tf.gather(self.y, ew_indices_y + 1) - tf.gather(self.y, ew_indices_y)) * (tf.gather(self.z, ew_indices_z + 1) - tf.gather(self.z, ew_indices_z - 1)) / 2;
    flow_east = .5 * self.rho * area_east * (tf.gather_nd(u_old, self.indices(ew_tail_indices_x, ew_indices_y, ew_indices_z)) + \
  ...
  def pressure(self, Apu, Apv, Apw, pressure_iter):
    ...
    App = tf.where(tf.cast(tf.scatter_nd([[1,1,1]], tf.constant([1], dtype = tf.float32), App.shape), dtype = tf.bool), 1e30 * tf.ones_like(App), App);
